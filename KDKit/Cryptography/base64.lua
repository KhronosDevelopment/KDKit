...
-- Source: https://github.com/Reselim/Base64/blob/8cd0d65a898794d41ffa823fcdf8b254574ec5f4/Base64.lua
-- KhronosDevelopment slightly modified the API
...
local function encode(input: string): string
    return buffer.tostring(encodeBuffer(buffer.fromstring(input)))
end

local function decodeBuffer(input: buffer): buffer
    local inputLength = buffer.len(input)
...
local function decode(input: string): string
    return buffer.tostring(decodeBuffer(buffer.fromstring(input)))
end
